...
public class OAuth2ObjectTest extends BaseTest {
    ...
    public void cannotUpdateClientByUnauthorizedUsers() {
        ...
        makeUnauthorizedOperationAndVerify(getClientPath("nuxeo-client-5"), RequestType.PUT, data);
    }
    ...
    /**
     * Make the CRUD operation and check the response. An operation is succeeds if it's response status worth the
     * expectedStatus param and it's body match the expected {@link JsonNode} if it's exists.
     * <p>
     * {@link org.skyscreamer.jsonassert.JSONAssert} is used instead of {@link JsonNode#equals(Object)} to avoid the
     * order check in the case of arrays.
     *
