...
public class MongoDBChecker implements BackingChecker {
    ...
    public void check(ConfigurationGenerator cg) throws ConfigurationException {
        ...
        if (configFile.exists()) {
            ...
            try (MongoClient mongoClient = MongoDBConnectionHelper.newMongoClient(config, builder ->
                    builder.serverSelectionTimeout((int) TimeUnit.SECONDS.toMillis(getCheckTimeoutInSeconds(cg)))
     ...
     * @since 11.1
     */
    public <T extends Descriptor> T getDescriptor(File file, Class<T> descriptor) throws ConfigurationException {
        XMap xmap = new XMap();
        xmap.register(descriptor);
        try (InputStream inStream = new FileInputStream(file)) {
            return (T) xmap.load(inStream);
        } catch (IOException e) {
            throw new ConfigurationException("Unable to load the configuration for " + descriptor.getSimpleName(), e);
        }
    }
